...
def parse_data(input_path='trialdata.csv'):
    ...
    decision_feedback_df = pd.DataFrame(columns=['subId', 'overall', 'positive', 'negative', 'photo-issue', 'others'])
    likert_counter = 0
    ...
    for index, row in trial.iterrows():
        ...
        likert_data_df.to_csv(task_name+'_likert_data.csv')
        demo_survey_df.to_csv(task_name+'_demo_survey.csv')
        feedback_df.to_csv(task_name+'_feedback_for_hit.csv')
        decision_feedback_df.to_csv(task_name+'_decision_feedback.csv')
